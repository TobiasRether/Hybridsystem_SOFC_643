...
def combustion_chamber3(__compressor_in_bulk, __compressor_out_bulk, __fuel_phase, __menext, __m_cooler, __p_booster,
    ...
    """ Generic numeric Model of combustion chamber for Gas Turbine w/ external ccoling systen
     parametrized by input values __compressor_in_bulk, __compressor_out_bulk, __fuel_phase, __menext, __m_cooler, __p_booster,
                        __t_hex_out, __eta_cc, __dp_cc, __tt1, __m_fuel, __controller
    ...
    __compressor_out_bulk - Compressor Outlet/Oxidizer Combustion Chamber Inlet Cantera Quantity Object
    ...
    bulk_vector - vector of following Cantera Objects:
        ...
        2 - __compressor_out_bulk - Cantera Object representing Compressor Outlet bulk
    ...
    hv2 = sensible_enthalpy(__compressor_out_bulk.phase, ref_type=1)[0]/1000
    mv2 = __compressor_out_bulk.mass
    pv2 = __compressor_out_bulk.phase.P

    cooler_in_bulk = ct.Quantity(__compressor_out_bulk.phase, mass=__m_cooler)
    cooler_out_bulk = ct.Quantity(__compressor_out_bulk.phase, mass=__m_cooler)
    ...
    fuel_to_air_ratio_stoech = fuel_to_air_ratio2(__compressor_out_bulk.phase, __fuel_phase, phi)
    ...
    for i in range(n+1):
        ...
        flue_gas_bulk = __compressor_out_bulk + fuel_bulk
    ...
    bulk_vector = [__compressor_in_bulk, __compressor_out_bulk, cooler_in_bulk, cooler_out_bulk, fuel_bulk, flue_gas_bulk]

    attribute_vector = []

    df1 = create_state_dataframe(__compressor_out_bulk, "Oxidator Combustion Chamber Inlet")
...
def turbine(turbine_inlet_bulk, pt2, etat):

    tt1 = turbine_inlet_bulk.phase.T
    pt1 = turbine_inlet_bulk.phase.P
    mt1 = turbine_inlet_bulk.mass
    st1 = turbine_inlet_bulk.phase.s
    ht1 = turbine_inlet_bulk.phase.h

    turbine_outlet_bulk = ct.Quantity(turbine_inlet_bulk.phase, mass=mt1)
    turbine_outlet_bulk.SP = st1, pt2
    tt2_is = turbine_outlet_bulk.phase.T
    ht2_is = turbine_outlet_bulk.phase.h

    ht2 = (ht2_is - ht1) * etat + ht1

    turbine_outlet_bulk.HP = ht2, pt2
    tt2 = turbine_outlet_bulk.phase.T

    turbine_power = - mt1 * (ht2 - ht1) / 1000

    df1 = create_state_dataframe(turbine_inlet_bulk, "Turbine Inlet")
    df2 = create_state_dataframe(turbine_outlet_bulk, "Turbine Outlet")

    bulk_vector = [turbine_inlet_bulk, turbine_outlet_bulk]

    attribute_vector = [mt1, pt1, tt1, pt2, tt2_is, tt2, turbine_power]
